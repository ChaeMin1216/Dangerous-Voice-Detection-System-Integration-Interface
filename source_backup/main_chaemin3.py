...
from audio_handler_chaemin import process_audio_file
import os
import asyncio
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        if device_ip not in audio_id_map:
            audio_id_map[device_ip] = audio_id_counter
            audio_id_counter += 1

        Audio_Id = audio_id_map[device_ip]
        print(f"Assigned Audio_Id {Audio_Id} to IP {device_ip}")
...
# 파일 모니터링 및 처리
async def monitor_and_process(directory: str):
    ...
    while True:
        for file_path in Path(directory).iterdir():
            if file_path.is_file() and file_path.name not in processed_files:
                ...
                try:
                    # 장치 식별자 가져오기
                    device_ip = websocket_manager.connected_websockets[0].client.host
                    Audio_Id = audio_id_map.get(device_ip, "unknown")
...
async def startup_event():
    ...
    # 파일 모니터링 작업을 비동기로 시작
    asyncio.create_task(monitor_and_process(raw_audio_directory))

if __name__ == "__main__":
    uvicorn.run("main:app", host="0.0.0.0", port=15410, reload=True)
